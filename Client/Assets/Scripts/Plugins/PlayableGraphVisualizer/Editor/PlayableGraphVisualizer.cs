...
namespace GraphVisualizer {
    public class PlayableGraphVisualizer : Graph {
        ...
        protected override IEnumerable<Node> GetChildren(Node node) {
            ...
            if (node is PlayableNode) return GetInputsFromPlayableNode((Playable)node.content);
            if (node is PlayableOutputNode) return GetInputsFromPlayableOutputNode((PlayableOutput)node.content);
            return new List<Node>();
        }
        private List<Node> GetInputsFromPlayableNode(Playable h) {
            var inputs = new List<Node>();
            if (!h.IsValid()) return inputs;
        ...
        private List<Node> GetInputsFromPlayableOutputNode(PlayableOutput h) {
            var inputs = new List<Node>();
            if (!h.IsOutputValid()) return inputs;
            Playable playable = h.GetSourcePlayable();
            if (!playable.IsValid()) return inputs;
        ...
        private PlayableNode CreateNodeFromPlayable(Playable h, float weight) {
            ...
            if (type == typeof(AnimationClipPlayable)) return new AnimationClipPlayableNode(h, weight);
            if (type == typeof(AnimationLayerMixerPlayable)) return new AnimationLayerMixerPlayableNode(h, weight);
