    ...
    public class DefaultGraphRenderer : IGraphRenderer {
        ...
        public void DrawRect(Rect rect, Color color, string text, bool active, bool selected = false) {
            ...
            if (selected) {
                ...
                GUI.Box(new Rect(rect.x - t, rect.y - t, rect.width + 2 * t, rect.height + 2 * t), string.Empty, m_NodeRectStyle);
            }
            if (active) {
                GUI.color = s_ActiveNodeColor;
                GUI.Box(new Rect(rect.x - s_ActiveNodeThickness, rect.y - s_ActiveNodeThickness, rect.width + 2 * s_ActiveNodeThickness, rect.height + 2 * s_ActiveNodeThickness), string.Empty, m_NodeRectStyle);
            }
        ...
        private void DrawGraph(IGraphLayout graphLayout, Rect drawingArea, GraphSettings graphSettings) {
            ...
            drawingArea = new Rect(drawingArea.x + s_BorderSize, drawingArea.y + s_BorderSize, drawingArea.width - s_BorderSize * 2, drawingArea.height - s_BorderSize * 2);
            ...
            foreach (Vertex v in graphLayout.vertices) {
                ...
                bool currentSelection = (m_SelectedNode != null) && v.node.content.Equals(m_SelectedNode.content); // Make sure to use Equals() and not == to call any overriden comparison operator in the content type.
        ...
        private static Vector2 ComputeNodeSize(Vector2 scale, GraphSettings graphSettings) {
            ...
            var nodeSize = new Vector2(graphSettings.maximumNormalizedNodeSize * scale.x - extraTickness, graphSettings.maximumNormalizedNodeSize * scale.y - extraTickness);
        ...
        private static void DrawEdge(Vector2 a, Vector2 b, float weight) {
            ...
            Handles.DrawBezier(points[0], points[1], tangents[0], tangents[1], Color.Lerp(s_EdgeColorMin, s_EdgeColorMax, weight), null, 5f);
        }
    }
}